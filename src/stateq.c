...
void
stateq(int id, struct grid *g, molData *m, int ispec, inputPars *par, double *phot, double *vfac, double *ds){
  int t,s,iter;
  ...
  while((diff>TOL && iter<MAXITER) || iter<5){
    getjbar(id,m,g,par,jbar,phot,vfac,ds);
    getmatrix(id,matrix,m,g,ispec,jbar);
...
void
getmatrix(int id, gsl_matrix *matrix, molData *m, struct grid *g, int ispec, double *jbar){
  int p,t,k,l,ipart;
  struct getmatrix {
    double *ctot;
    gsl_matrix * colli;
  ...
  for(t=0;t<m[ispec].nline;t++){
    k=m[ispec].lau[t];
    l=m[ispec].lal[t];
    gsl_matrix_set(matrix, k, k, gsl_matrix_get(matrix, k, k)+m[ispec].beinstu[t]*jbar[t]+m[ispec].aeinst[t]);
    gsl_matrix_set(matrix, l, l, gsl_matrix_get(matrix, l, l)+m[ispec].beinstl[t]*jbar[t]);
    gsl_matrix_set(matrix, k, l, gsl_matrix_get(matrix, k, l)-m[ispec].beinstl[t]*jbar[t]);
    gsl_matrix_set(matrix, l, k, gsl_matrix_get(matrix, l, k)-m[ispec].beinstu[t]*jbar[t]-m[ispec].aeinst[t]);
  }
