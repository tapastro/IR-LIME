...
kappa(molData *m, struct grid *g, inputPars *par, int s){
  ...
  if(par->dust == NULL){
    ...
    rewind(fp);
    if(i<1){
      if(!silent) bail_out("Error: No opacities read");
    } else {
      ...
      for(j=0;j<m[s].nline && m[s].freq[j] > 0;j++) {
        ...
        if(par->abundancesAreAbsolute){
          ...
          if(m[i].npart == 1 && count[0] == 1){
            ...
            if(!flag){
              ...
              //This may need to be more explicit if the warning is not seen during runtime
              g[id].nmol[ispec] = g[id].abun[ispec]*(g[id].dens[0]+g[id].dens[1]);
